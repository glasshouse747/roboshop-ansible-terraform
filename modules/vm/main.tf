...
resource "null_resource" "ansible" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ansible-pull -i localhost, -U https://github.com/glasshouse747/roboshop-ansible-terraform.git roboshop.yml -e role_name=${local.role_name} -e env=dev -e token=${var.token}"
    ]
  }
